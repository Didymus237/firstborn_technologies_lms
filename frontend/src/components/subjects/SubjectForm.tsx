...
export function SubjectForm({
  ...
  useEffect(() => {
    if (initialData) {
      // Map the teacher field to an array of IDs (strings) supporting both array and single object types robustly
      let teacherIds: string[] = [];
      if (initialData.teacher) {
        if (Array.isArray(initialData.teacher)) {
          teacherIds = initialData.teacher.map((t: any) =>
            typeof t === "object" ? t._id : t
          );
        } else {
          const t = initialData.teacher as any;
          teacherIds = [typeof t === "object" ? t._id : t];
        }
      }
  ...
  const onSubmit = async (values: SubjectFormValues) => {
    ...
    try {
      ...
      if (initialData) {
        await api.put(`/subjects/update/${initialData._id}`, payload);
