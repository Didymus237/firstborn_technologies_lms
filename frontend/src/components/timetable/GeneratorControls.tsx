  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const yearsData = Array.isArray(yearRes.data) ? yearRes.data : yearRes.data.academicYears || yearRes.data.years || [];
        setYears(yearsData);

        // Auto-select current year
        if (!selectedYear) {
          const current = yearsData.find((y: any) => y.isCurrent || y.iscurrent) || yearsData[0];
