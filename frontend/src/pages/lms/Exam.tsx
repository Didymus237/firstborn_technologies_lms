...
const Exam = () => {
  ...
  const totalPoints = submission && exam?.questions ? exam.questions.length : 0;
  const percentage =
  ...
  const fetch = async () => {
    try {
      ...
      if (isStudent) {
        try {
          const resultRes = await api.get(`/exams/${id}/results`);
  ...
  const handleToggleStatus = async () => {
    try {
      const { data } = await api.put(`/exams/${id}/status`);
